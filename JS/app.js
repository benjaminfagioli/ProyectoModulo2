...
const cargarTabla=()=>{
    ...
    datos.map((item)=>{
        ...
        <button class="btn btn-outline-warning" onclick="mostrarModal(${item.id})"><i class="fa 
        fa-pencil" aria-hidden="true"></i></button>
        <button class="btn btn-outline-danger" onclick="borrarGift(${item.id})"><i class="fa 
...
const agregarGift=(event)=>{
 ...
 window.borrarGift = (id)=>{
    ...
    let validar = confirm(
        'Está seguro/a que quiere eliminar la gift card ${datos[inde].gift}?');
